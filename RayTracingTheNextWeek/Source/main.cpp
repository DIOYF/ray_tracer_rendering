...
color ray_color(const ray &r,const color& background, const hittable_list& world, int depth)
{
	...
	ray scattered;
	color attenuation;
	color emitted =  rec_info.mat_ptr->emitted(rec_info.u, rec_info.v,rec_info.p);

	if (rec_info.mat_ptr->scatter(r, rec_info, attenuation, scattered)) // 
		return emitted + attenuation * ray_color(scattered, background,world, depth-1); // attenuation multiple  add
	return emitted;
