...
public:
	...
	vec3 operator-() const { return vec3(-e[0], -e[1], -e[2]); } // 常量函数有权读取外部内容，但无权修改它们
...
// vec3 Utility Functions, 为相应的类定义新的扩展
inline std::ostream& operator<<(std::ostream& out, const vec3& v) {
	return out << v.e[0] << ' ' << v.e[1] << ' ' << v.e[2];
}

inline vec3 operator+(const vec3& u, const vec3& v) { // 常量引用不能再调用类的成员函数，
...
inline vec3 operator*(const vec3& u, const float t) { // 常量引用不能再调用类的成员函数，
	return vec3(u.e[0] * t, u.e[1] * t, u.e[2] * t);
}

inline vec3 operator*(const float t ,const vec3& u) { // 常量引用不能再调用类的成员函数，
	return vec3(u.e[0] * t, u.e[1] * t, u.e[2] * t);
}
