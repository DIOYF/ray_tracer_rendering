...
float sphere::pdf_value(const point3 & o, const vec3 & v) const {
	...
	float cos_theta_max = sqrt(1 - radius_value * radius_value / (center_value - o).length_squared());
	float solid_angle = 2 * pi * (1 - cos_theta_max);
	return 1 / (solid_angle+1e-7);
}
...
vec3 sphere::random(const point3& o) const {
	vec3 direction = center_value - o;
	float distance_squared = direction.length_squared();
	onb uvw;
	uvw.build_from_w(direction);
	return uvw.local(random_to_sphere(radius_value, distance_squared));
